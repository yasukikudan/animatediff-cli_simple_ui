...
@dataclass
class AnimationPipelineImg2ImgControlnetOutput(BaseOutput):
...
class AnimationPipelineImg2ImgControlnet(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def __call__(
      ...
      # 5. Prepare latent variables
        ...
        with self.progress_bar(total=total_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for z in range(0, batch_size, batch_size_per_subbatch):
                    ...
                    # controlnet_cond を作成
                    subbatch_controlnet_cond = torch.cat([controlnet_image[:, :, subbatch_latents_context]] * 3, dim=1)
                ...
                #down_block_res_samples = [torch.cat([d[0:1],torch.zeros_like(d[0:1])]) for d in down_block_res_samples]
                #mid_block_res_sample = torch.cat([torch.zeros_like(mid_block_res_sample[0:1]), mid_block_res_sample[1:2]])
                ...
                for context in context_scheduler(
                    ...
                    if len(processed_batch_indices)>=video_length:
                        progress_bar.update()
                        continue

                    processed_batch_indices.update(context)
                    ##コンテストの最初と最後のインデックスを比較、最後のインデックスが最初のインデックスより小さい場合はスキップする
        ...
        self.unet.to(torch.device("cpu"))
        self.controlnet.to(torch.device("cpu"))
        letents=latents.to(torch.device("cpu"))
        torch.cuda.empty_cache()
        ...
        return AnimationPipelineImg2ImgControlnetOutput(videos=video)
