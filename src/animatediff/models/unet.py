...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        if down_block_additional_residuals is not None:
            ...
            print(down_block_res_samples[0].shape, down_block_additional_residuals[0].shape)

            for down_block_res_sample, down_block_additional_residual in zip(
                down_block_res_samples, down_block_additional_residuals
            ):
                print("down_block_res_sample, down_block_additional_residual")
                print(down_block_res_sample.shape, down_block_additional_residual.shape)
                if residuals_ratio is None:
                    down_block_res_sample += down_block_additional_residual
                else:
                    down_block_res_sample = down_block_res_sample * (1.0 - residuals_ratio) + down_block_additional_residual * residuals_ratio
                new_down_block_res_samples += (down_block_res_sample,)

            print("down_block_res_samples",down_block_res_samples[0].shape,len(down_block_res_samples))
            print("new_down_block_res_samples",new_down_block_res_samples[0].shape,len(new_down_block_res_samples))
            down_block_res_samples = new_down_block_res_samples


        if mid_block_additional_residual is not None:
            print("sample", sample.shape)
            print("mid_block_additional_residual", mid_block_additional_residual.shape)
            if residuals_ratio is  None:
                sample += mid_block_additional_residual
            else:
                sample = sample * (1.0 - residuals_ratio) + mid_block_additional_residual * residuals_ratio
        ...
        print("down_block_res_sample")
        print(len(down_block_res_samples))
        #print(down_block_res_samples[-1])
        # up
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if not is_final_block and forward_upsample_size :
                print("upsample_size", len(down_block_res_samples))
